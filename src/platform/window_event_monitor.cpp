...
bool WindowEventMonitor::Start(WindowCallback minimize_start_callback,
  ...
  if (minimize_hook_ == nullptr || restore_hook_ == nullptr || show_hook_ == nullptr ||
    ...
    LogDebug(L"WinEvent", L"SetWinEventHook failed error=" + std::to_wstring(GetLastError()));
    Stop();
    return false;
  }

  LogDebug(L"WinEvent", L"Started WinEvent hooks and shell hook window");
  return true;
}
...
void WindowEventMonitor::HandleMinimizeStart(HWND window) {
  LogTrace(L"WinEvent", L"HandleMinimizeStart " + WindowBrief(window));
...
void WindowEventMonitor::HandleRestoreStart(HWND window) {
  LogTrace(L"WinEvent", L"HandleRestoreStart " + WindowBrief(window));
  ...
  if (msg == shell_hook_message_ && shell_hook_message_ != 0) {
    ...
    if (nCode == HSHELL_GETMINRECT) {
      ...
      if (info != nullptr) {
        ...
        LogTrace(L"WinEvent", L"ShellHook HSHELL_GETMINRECT " + WindowBrief(window));
        WINDOWPLACEMENT wp{};
        wp.length = sizeof(wp);
        if (GetWindowPlacement(window, &wp)) {
          if (wp.showCmd == SW_SHOWMINIMIZED || wp.showCmd == SW_MINIMIZE || IsIconic(window)) {
            ...
            LogTrace(L"WinEvent", L"ShellHook ignored because window is not minimized showCmd=" +
                                      std::to_wstring(wp.showCmd) + L" " + WindowBrief(window));
          }
        } else {
          LogTrace(L"WinEvent", L"ShellHook GetWindowPlacement failed error=" +
                                    std::to_wstring(GetLastError()) + L" " + WindowBrief(window));
        }
      }
    }
  }
...
void WindowEventMonitor::OnWinEvent(DWORD event, HWND window, LONG object_id, LONG child_id) {
  if (window == nullptr) {
    LogTrace(L"WinEvent", L"OnWinEvent ignored null window event=" + EventName(event));
    return;
  }

  LogTrace(L"WinEvent", L"OnWinEvent event=" + EventName(event) + L" object_id=" +
                            std::to_wstring(object_id) + L" child_id=" + std::to_wstring(child_id) +
                            L" " + WindowBrief(window));
  ...
  if (object_id != OBJID_WINDOW || child_id != CHILDID_SELF) {
    LogTrace(L"WinEvent", L"OnWinEvent ignored non-window object event=" + EventName(event) +
