...
constexpr float kContentBottomInset = 14.0f;
...
bool CompactButton(const char* id, const char* label, const ImVec2& size, ImFont* font, float scale,
  ...
  const float hover = Animate(ImGui::GetID(id), hovered ? 1.0f : 0.0f, 22.0f);
  ImDrawList* draw = ImGui::GetWindowDrawList();
  ...
  draw->AddRect(min, ImVec2(min.x + size.x, min.y + size.y),
                WithAlpha(Blend(kBorderColor, kAccentColor, hover * 0.45f), alpha), 3.0f * scale);
  const ImVec2 text_size = font->CalcTextSizeA(13.0f * scale, FLT_MAX, 0.0f, label);
  draw->AddText(font, 13.0f * scale,
                ImVec2(min.x + (size.x - text_size.x) * 0.5f,
...
bool LinkButton(const char* id, bool linked, float scale, float alpha) {
  ...
  draw->AddLine(ImVec2(left.x + 1.5f * scale, left.y),
                ImVec2(right.x - 1.5f * scale, right.y), icon, 1.4f * scale);
...
bool RadioChoice(const char* id, const char* label, bool selected, ImFont* font, float scale,
                 float alpha) {
  const ImVec2 min = ImGui::GetCursorScreenPos();
  ImGui::InvisibleButton(id, ImVec2(260.0f * scale, 22.0f * scale));
  const bool hovered = ImGui::IsItemHovered();
  ImDrawList* draw = ImGui::GetWindowDrawList();
  const ImVec2 center(min.x + 8.0f * scale, min.y + 11.0f * scale);
  draw->AddCircle(center, 6.0f * scale,
                  WithAlpha(hovered || selected ? kAccentColor : kSecondaryTextColor, alpha), 18,
                  1.5f * scale);
  if (selected) {
    draw->AddCircleFilled(center, 3.0f * scale, WithAlpha(kAccentColor, alpha), 14);
  }
  draw->AddText(font, 13.5f * scale, ImVec2(min.x + 22.0f * scale, min.y + 2.0f * scale),
                WithAlpha(kPrimaryTextColor, alpha), label);
  return ImGui::IsItemClicked();
}
...
const char* HotkeyResultText(HotkeyUpdateResult result) {
  switch (result) {
    case HotkeyUpdateResult::kSuccess: return "Hotkey updated";
    case HotkeyUpdateResult::kInvalid: return "Choose a valid key combination";
    case HotkeyUpdateResult::kDuplicate: return "That combination is already used by Genie Effect";
    case HotkeyUpdateResult::kUnavailable: return "Hotkey unavailable; another application may use it";
    case HotkeyUpdateResult::kSaveFailed: return "Could not save the hotkey setting";
  }
...
bool SettingsWindow::ActivateExistingInstance(DWORD timeout_ms) {
  ...
  do {
    ...
    if (window != nullptr) {
      DWORD_PTR ignored = 0;
      return SendMessageTimeoutW(window, kShowSettingsMessage, 0, 0,
...
bool SettingsWindow::Initialize(HINSTANCE instance, ToggleCallback toggle_callback,
                                SpeedCallback speed_callback, LinkCallback link_callback,
                                AdaptiveDurationCallback adaptive_duration_callback,
                                ...
                                ExclusionCallback exclusion_callback,
...
LRESULT CALLBACK SettingsWindow::PreviewWindowProc(HWND hwnd, UINT message, WPARAM w_param,
  ...
  switch (message) {
    ...
    case WM_MOUSEMOVE: {
      if (settings == nullptr || !settings->preview_dragging_ ||
...
void SettingsWindow::StartAnimationPreview() {
  ...
  preview_window_ = CreateWindowExW(
      WS_EX_APPWINDOW, kPreviewWindowClass, L"Genie Effect Preview",
      WS_POPUP | WS_SYSMENU | WS_MINIMIZEBOX, x, y, width, height, nullptr, nullptr, instance,
      this);
...
void SettingsWindow::RenderContents() {
  ...
  draw->AddRectFilled(window_origin,
                      ImVec2(window_origin.x + window_size.x, window_origin.y + window_size.y),
                      kBackgroundColor, 0.0f);
  // Slate border
  draw->AddRect(
      window_origin,
      ImVec2(window_origin.x + window_size.x - 1.0f, window_origin.y + window_size.y - 1.0f),
      kBorderColor, 0.0f);

  // Title
  draw->AddText(font_medium_, px(14.0f), window_point(px(18.0f), px(16.0f) + y_offset),
                WithAlpha(kPrimaryTextColor, content_alpha), "Genie Effect");

  // Minimize Button
  ImGui::SetCursorPos(ImVec2(window_size.x - px(54.0f), px(14.0f) + y_offset));
  if (MinimizeButton(scale, content_alpha)) {
    ShowWindow(hwnd_, SW_MINIMIZE);
  }

  // Close Button
  ImGui::SetCursorPos(ImVec2(window_size.x - px(34.0f), px(14.0f) + y_offset));
  if (CloseButton(scale, content_alpha)) {
    HandleCloseRequest();
  }

  draw->AddLine(window_point(0.0f, px(kHeaderHeight)),
                window_point(window_size.x, px(kHeaderHeight)),
                WithAlpha(kBorderColor, content_alpha));
  draw->AddRectFilled(window_point(0.0f, px(kHeaderHeight)),
                      window_point(px(kSidebarWidth), window_size.y),
                      WithAlpha(IM_COL32(25, 25, 27, 255), content_alpha));
  draw->AddLine(window_point(px(kSidebarWidth), px(kHeaderHeight)),
                window_point(px(kSidebarWidth), window_size.y),
                WithAlpha(kBorderColor, content_alpha));

  constexpr std::array pages = {
      std::pair{Page::kGeneral, "General"},
      std::pair{Page::kAnimation, "Animation"},
      std::pair{Page::kApplications, "Applications"},
      std::pair{Page::kWindowsIntegration, "Windows & system"},
      std::pair{Page::kHotkeys, "Hotkeys"},
      std::pair{Page::kDiagnostics, "Diagnostics & repair"},
      std::pair{Page::kAbout, "About"},
  };
  float navigation_y = px(kHeaderHeight + 14.0f);
  for (size_t index = 0; index < pages.size(); ++index) {
    const auto& [page, label] = pages[index];
    const ImVec2 item_min = window_point(px(10.0f), navigation_y);
    ImGui::SetCursorScreenPos(item_min);
    const std::string id = std::format("##settings_page_{}", index);
    ImGui::InvisibleButton(id.c_str(), ImVec2(px(kSidebarWidth - 20.0f), px(36.0f)));
    const bool hovered = ImGui::IsItemHovered();
    const bool selected = selected_page_ == page;
    ...
    if (selected || hovered) {
      draw->AddRectFilled(
          item_min, ImVec2(item_min.x + px(kSidebarWidth - 20.0f), item_min.y + px(36.0f)),
          WithAlpha(selected ? IM_COL32(42, 55, 70, 255) : IM_COL32(35, 35, 39, 255),
                    content_alpha),
          px(4.0f));
    }
    if (selected) {
      draw->AddRectFilled(item_min, ImVec2(item_min.x + px(3.0f), item_min.y + px(36.0f)),
                          WithAlpha(kAccentColor, content_alpha), px(2.0f));
    }
    draw->AddText(font_body_, px(13.5f), ImVec2(item_min.x + px(14.0f), item_min.y + px(9.0f)),
                  WithAlpha(selected ? kPrimaryTextColor : kSecondaryTextColor, content_alpha),
                  label);
    navigation_y += px(40.0f);
  }

  ImGui::SetCursorPos(ImVec2(px(kSidebarWidth + 1.0f), px(kHeaderHeight + 1.0f)));
  ImGui::PushStyleColor(ImGuiCol_ChildBg, Color(0x1E1E1F));
  ImGui::BeginChild(
      "##settings_page",
      ImVec2(window_size.x - px(kSidebarWidth + 2.0f),
             window_size.y - px(kHeaderHeight + kContentBottomInset)),
  ...
  if (selected_page_ == Page::kGeneral) {
    draw->AddText(font_title_, px(21.0f), point(px(24.0f), px(20.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "General");

    draw->AddText(font_small_, px(12.0f), point(px(24.0f), px(51.0f) + y_offset),
                  WithAlpha(kSecondaryTextColor, content_alpha),
                  "Basic app behavior and startup options");

    // Row 1: Animations (y = 84)
    draw->AddText(font_medium_, px(13.0f), point(px(24.0f), px(84.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Effect");
    draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(114.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Enable Genie animations");
    ImGui::SetCursorPos(ImVec2(size.x - px(24.0f) - px(36.0f), px(111.0f) + y_offset));
    ...
    draw->AddText(font_medium_, px(13.0f), point(px(24.0f), px(164.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Closing settings");
    ImGui::SetCursorPos(ImVec2(px(24.0f), px(194.0f) + y_offset));
    ...
    ImGui::SetCursorPos(ImVec2(px(24.0f), px(224.0f) + y_offset));
    ...
    draw->AddText(font_medium_, px(13.0f), point(px(24.0f), px(274.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Startup");
    draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(308.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Start with Windows");
    ImGui::SetCursorPos(ImVec2(size.x - px(24.0f) - px(36.0f), px(305.0f) + y_offset));
    ...
    draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(350.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Start minimized to tray");
    ImGui::SetCursorPos(ImVec2(size.x - px(24.0f) - px(36.0f), px(347.0f) + y_offset));
  ...
  if (selected_page_ == Page::kAnimation) {
    ...
    const ImVec2 min_value_pos(size.x - px(18.0f) - min_duration_size.x, px(92.0f) + y_offset);
    if (numeric_edit_field_ == 0) {
      ImGui::SetCursorPos(ImVec2(size.x - px(86.0f), px(86.0f) + y_offset));
      ...
      draw->AddText(font_medium_, px(13.0f), point(min_value_pos.x, min_value_pos.y),
    ...
    ImGui::SetCursorPos(ImVec2(size.x - px(18.0f) - min_duration_size.x - slider_w - px(8.0f),
                               px(92.0f) + y_offset));
    ...
    ImGui::SetCursorPos(ImVec2(size.x - px(18.0f) - px(24.0f), px(112.0f) + y_offset));
    ...
    const ImVec2 restore_value_pos(size.x - px(18.0f) - restore_duration_size.x,
                                   px(132.0f) + y_offset);
    if (numeric_edit_field_ == 1) {
      ImGui::SetCursorPos(ImVec2(size.x - px(86.0f), px(126.0f) + y_offset));
      ImGui::SetNextItemWidth(px(68.0f));
      ImGui::SetKeyboardFocusHere();
      ...
      draw->AddText(font_medium_, px(13.0f), point(restore_value_pos.x, restore_value_pos.y),
    ...
    // Slider (Right-aligned next to value text)
    ImGui::SetCursorPos(ImVec2(size.x - px(18.0f) - restore_duration_size.x - slider_w - px(8.0f),
                               px(132.0f) + y_offset));
    ...
    draw->AddText(font_body_, px(13.5f), point(px(18.0f), px(174.0f) + y_offset),
    ...
    ImGui::SetCursorPos(ImVec2(size.x - px(24.0f) - px(72.0f), px(18.0f) + y_offset));
    ...
    draw->AddText(font_medium_, px(13.0f), point(px(18.0f), px(270.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Test the animation");
    draw->AddText(font_small_, px(12.0f), point(px(18.0f), px(296.0f) + y_offset),
                  WithAlpha(kSecondaryTextColor, content_alpha),
                  "Opens a real window, minimizes it, restores it, then closes it.");
    ImGui::SetCursorPos(ImVec2(size.x - px(18.0f) - px(112.0f), px(266.0f) + y_offset));
    ...
    draw->AddText(font_medium_, px(13.0f), point(px(18.0f), px(350.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Motion");
    draw->AddText(font_body_, px(13.5f), point(px(18.0f), px(382.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha),
                  "Adapt duration to animation distance");
    ImGui::SetCursorPos(
        ImVec2(size.x - px(18.0f) - px(36.0f), px(379.0f) + y_offset));
    ...
    if (Toggle("##adaptive_duration", &adaptive_duration_, scale, content_alpha)) {
      const bool saved = !adaptive_duration_callback_ ||
                         adaptive_duration_callback_(adaptive_duration_);
    ...
    draw->AddText(font_body_, px(13.5f), point(px(18.0f), px(426.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Animation style");
    ImGui::SetCursorPos(
        ImVec2(size.x - px(18.0f) - px(150.0f), px(422.0f) + y_offset));
    ...
    if (ImGui::BeginCombo("##animation_style", animation_style_.c_str())) {
      for (const char* name : style_names) {
        ...
        if (ImGui::Selectable(name, selected)) {
          ...
          const bool saved = !animation_style_callback_ ||
                             animation_style_callback_(animation_style_);
    ...
    const auto easing_combo = [&](const char* id, const char* label, std::string* value,
      ...
      draw->AddText(font_body_, px(13.5f), point(px(18.0f), px(row_y) + y_offset),
                    WithAlpha(kPrimaryTextColor, content_alpha), label);
      ImGui::SetCursorPos(ImVec2(size.x - px(18.0f) - px(150.0f),
      ...
      if (ImGui::BeginCombo(id, value->c_str())) {
        for (const char* name : easing_names) {
          ...
          if (ImGui::Selectable(name, selected)) {
            ...
            *value = name;
            const bool saved = !easing_callback_ ||
                               easing_callback_(minimize ? *value : minimize_easing_,
    ...
    easing_combo("##minimize_easing", "Minimize easing", &minimize_easing_, 470.0f, true);
    easing_combo("##restore_easing", "Restore easing", &restore_easing_, 514.0f, false);

    draw->AddText(font_medium_, px(13.0f), point(px(18.0f), px(562.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Visual effects");
    draw->AddText(font_body_, px(13.5f), point(px(18.0f), px(594.0f) + y_offset),
    ...
    const ImVec2 strength_value_pos(size.x - px(70.0f), px(594.0f) + y_offset);
    if (numeric_edit_field_ == 2) {
      ImGui::SetCursorPos(ImVec2(size.x - px(82.0f), px(588.0f) + y_offset));
      ImGui::SetNextItemWidth(px(64.0f));
      ImGui::SetKeyboardFocusHere();
      const bool apply = ImGui::InputFloat("##strength_input", &numeric_edit_value_, 0, 0,
                                           "%.2f", ImGuiInputTextFlags_EnterReturnsTrue);
      if (apply) {
      ...
      draw->AddText(font_medium_, px(13.0f), point(strength_value_pos.x, strength_value_pos.y),
    ...
    ImGui::SetCursorPos(
        ImVec2(size.x - px(290.0f), px(590.0f) + y_offset));
    float proposed_strength = genie_strength_;
    const bool strength_active =
        Slider("##genie_strength", &proposed_strength, 0.25f, 1.0f, px(200.0f), scale,
               content_alpha, 1.0f, 0.01f);
    ...
    draw->AddText(font_body_, px(13.5f), point(px(18.0f), px(638.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Fade during animation");
    ImGui::SetCursorPos(ImVec2(size.x - px(214.0f), px(631.0f) + y_offset));
    ...
    draw->AddText(font_body_, px(13.5f), point(px(18.0f), px(682.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Show taskbar target indicator");
    ImGui::SetCursorPos(ImVec2(size.x - px(60.0f), px(679.0f) + y_offset));
    bool proposed_indicator = show_target_indicator_;
    if (Toggle("##target_indicator", &proposed_indicator, scale, content_alpha)) {
      const bool saved = !target_indicator_callback_ ||
                         target_indicator_callback_(proposed_indicator);
      if (saved) show_target_indicator_ = proposed_indicator;
      RecordSaveResult(saved);
    }
    draw->AddText(font_small_, px(12.0f), point(px(18.0f), px(710.0f) + y_offset),
                  WithAlpha(kSecondaryTextColor, content_alpha),
  ...
  if (selected_page_ == Page::kApplications) {
    ...
    for (const std::string& active : cached_active_apps_) {
      auto it = std::find_if(items.begin(), items.end(),
                             [&active](const AppItem& item) { return ExecutableNamesEqual(item.name, active); });
      if (it != items.end()) {
        it->is_active = true;
      } else {
    ...
    ImGui::SetCursorPos(ImVec2(px(24.0f), px(108.0f) + y_offset));
    ImGui::SetNextItemWidth(size.x - px(24.0f * 2.0f));
    ImGui::InputTextWithHint("##app_search", "Search applications...", exclusion_input_.data(),
                             exclusion_input_.size());
    ImGui::PopStyleColor(2);
    ImGui::PopStyleVar(2);

    ImGui::SetCursorPos(ImVec2(px(24.0f), px(146.0f) + y_offset));
    ImGui::PushStyleColor(ImGuiCol_ChildBg, Color(0x191A1E));
    if (ImGui::BeginChild("##excluded_list", ImVec2(size.x - px(48.0f), px(330.0f)),
      ...
      if (filtered_items.empty()) {
        ...
        for (size_t i = 0; i < filtered_items.size(); ++i) {
          const float row_y = px(4.0f + static_cast<float>(i) * 28.0f);
          ImGui::SetCursorPos(ImVec2(px(8.0f), row_y + px(4.0f)));
          ...
          ImGui::SetCursorPos(ImVec2(child_width - px(52.0f), row_y + px(2.0f)));
          ...
          if (Toggle(toggle_id.c_str(), &excluded, scale, content_alpha)) {
            if (exclusion_callback_) {
              if (exclusion_callback_(filtered_items[i].name, excluded)) {
                ...
                if (excluded) {
                  if (std::find(excluded_applications_.begin(), excluded_applications_.end(), filtered_items[i].name) == excluded_applications_.end()) {
    ...
    if (!visible_error.empty()) {
      draw->AddText(font_small_, px(11.5f), point(px(24.0f), px(486.0f) + y_offset),
  ...
  if (selected_page_ == Page::kHotkeys) {
    ...
    draw->AddText(font_body_, px(14.0f), point(px(24.0f), px(56.0f) + y_offset),
                  WithAlpha(kSecondaryTextColor, content_alpha),
                  "Click Change, then press the new key combination.");
    constexpr std::array labels = {
        "Toggle Genie Effect", "Open Settings", "Repair Windows",
    };
    for (size_t index = 0; index < labels.size(); ++index) {
      const float row_y = 96.0f + static_cast<float>(index) * 66.0f;
      draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(row_y) + y_offset),
      ...
      const ImU32 binding_color =
          hotkey_available_[index] || hotkeys_[index].virtual_key == 0
              ? kSecondaryTextColor
              : IM_COL32(235, 120, 120, 255);
      draw->AddText(font_medium_, px(13.0f), point(px(24.0f), px(row_y + 25.0f) + y_offset),
                    WithAlpha(binding_color, content_alpha), binding_text.c_str());
      ImGui::SetCursorPos(ImVec2(size.x - px(172.0f), px(row_y + 12.0f) + y_offset));
      ...
      ImGui::SetCursorPos(ImVec2(size.x - px(88.0f), px(row_y + 12.0f) + y_offset));
    ...
    if (!hotkey_feedback_.empty()) {
      draw->AddText(font_small_, px(12.0f), point(px(24.0f), px(96.0f + static_cast<float>(labels.size()) * 66.0f + 12.0f) + y_offset),
  ...
  if (selected_page_ == Page::kWindowsIntegration) {
    ...
    draw->AddText(font_small_, px(12.0f), point(px(24.0f), px(51.0f) + y_offset),
                  WithAlpha(kSecondaryTextColor, content_alpha),
                  "System behavior and power usage");
    draw->AddText(font_medium_, px(13.0f), point(px(24.0f), px(82.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Windows behavior");
    draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(116.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha),
                  "Disable animations while a fullscreen application is active");
    ImGui::SetCursorPos(
        ImVec2(size.x - px(24.0f) - px(36.0f), px(113.0f) + y_offset));
    ...
    draw->AddText(font_small_, px(12.0f), point(px(24.0f), px(145.0f) + y_offset),
                  WithAlpha(kSecondaryTextColor, content_alpha),
                  "Normal maximized windows continue to use Genie animations.");

    draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(190.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha),
                  "Follow the Windows animation preference");
    ImGui::SetCursorPos(ImVec2(size.x - px(60.0f), px(187.0f) + y_offset));
    ...
    draw->AddText(font_medium_, px(13.0f), point(px(24.0f), px(244.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Power");
    draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(278.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Reduce effects on battery");
    ImGui::SetCursorPos(
        ImVec2(size.x - px(24.0f) - px(36.0f), px(275.0f) + y_offset));
    ...
    if (Toggle("##reduce_on_battery", &proposed_reduce_on_battery, scale, content_alpha)) {
      const bool saved = !power_behavior_callback_ ||
                         power_behavior_callback_(proposed_reduce_on_battery,
                                                  disable_effects_battery_saver_);
    ...
    draw->AddText(font_body_, px(13.5f), point(px(24.0f), px(324.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha),
                  "Disable effects in battery saver");
    ImGui::SetCursorPos(
        ImVec2(size.x - px(24.0f) - px(36.0f), px(321.0f) + y_offset));
    bool proposed_disable_in_saver = disable_effects_battery_saver_;
    if (Toggle("##disable_in_battery_saver", &proposed_disable_in_saver, scale,
               content_alpha)) {
      const bool saved = !power_behavior_callback_ ||
                         power_behavior_callback_(reduce_effects_on_battery_,
                                                  proposed_disable_in_saver);
  ...
  if (selected_page_ == Page::kDiagnostics) {
    ...
    draw->AddText(font_title_, px(21.0f), point(px(24.0f), px(20.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), "Diagnostics");
    ...
    for (size_t index = 0; index < rows.size(); ++index) {
      const float row_y = 62.0f + static_cast<float>(index) * 24.0f;
      draw->AddText(font_body_, px(13.0f), point(px(24.0f), px(row_y) + y_offset),
                    WithAlpha(kSecondaryTextColor, content_alpha), rows[index].first);
      draw->AddText(font_medium_, px(13.0f), point(px(180.0f), px(row_y) + y_offset),
                    WithAlpha(kPrimaryTextColor, content_alpha), rows[index].second->c_str());
    }
    constexpr std::array actions = {
        std::pair{"Open Log Folder", DiagnosticsAction::kOpenLogFolder},
        std::pair{"Repair Windows", DiagnosticsAction::kRepairWindows},
        std::pair{"Restart Renderer", DiagnosticsAction::kRestartRenderer},
    };
    float button_x = px(24.0f);
    for (size_t index = 0; index < actions.size(); ++index) {
      ImGui::SetCursorPos(ImVec2(button_x, px(474.0f) + y_offset));
      const std::string id = std::format("##diagnostics_action_{}", index);
      if (CompactButton(id.c_str(), actions[index].first, ImVec2(px(132.0f), px(30.0f)),
                        font_small_, scale, content_alpha)) {
        const bool succeeded = diagnostics_action_callback_ &&
                               diagnostics_action_callback_(actions[index].second);
    ...
    if (diagnostics_.safe_mode == "On") {
      ImGui::SetCursorPos(ImVec2(px(24.0f), px(514.0f) + y_offset));
      if (CompactButton("##exit_safe_mode", "Start Normally", ImVec2(px(132.0f), px(30.0f)),
                        font_small_, scale, content_alpha)) {
        const bool succeeded = diagnostics_action_callback_ &&
                               diagnostics_action_callback_(DiagnosticsAction::kExitSafeMode);
        diagnostics_feedback_ = succeeded ? "Normal animation runtime started"
                                          : "Could not leave Safe Mode";
        last_diagnostics_refresh_ms_ = 0;
      }
    }
    if (!diagnostics_feedback_.empty()) {
      draw->AddText(font_small_, px(12.0f), point(px(170.0f), px(522.0f) + y_offset),
                    WithAlpha(kSecondaryTextColor, content_alpha), diagnostics_feedback_.c_str());
    }
  }

  if (selected_page_ == Page::kAbout) {
    const char* page_title = "About";
    const char* page_description = "Genie Effect for Windows";
    draw->AddText(font_title_, px(21.0f), point(px(24.0f), px(20.0f) + y_offset),
                  WithAlpha(kPrimaryTextColor, content_alpha), page_title);
    draw->AddText(font_body_, px(14.0f), point(px(24.0f), px(58.0f) + y_offset),
                  WithAlpha(kSecondaryTextColor, content_alpha), page_description);
  }

  if (!save_feedback_.empty() && GetTickCount64() < save_feedback_until_ms_) {
    const ImU32 color = save_feedback_error_ ? IM_COL32(235, 120, 120, 255)
                                             : IM_COL32(120, 205, 145, 255);
    const ImVec2 text_size = ImGui::CalcTextSize(save_feedback_.c_str());
    draw->AddRectFilled(point(size.x - text_size.x - px(50.0f), size.y - px(42.0f) + y_offset),
                        point(size.x - px(18.0f), size.y - px(14.0f) + y_offset),
                        WithAlpha(IM_COL32(34, 37, 43, 255), content_alpha), px(6.0f));
    draw->AddText(font_small_, px(12.0f),
                  point(size.x - text_size.x - px(34.0f), size.y - px(35.0f) + y_offset),
...
LRESULT CALLBACK SettingsWindow::WindowProc(HWND hwnd, UINT message, WPARAM w_param,
  ...
  if (settings != nullptr && message == WM_LBUTTONDOWN) {
    ...
    if (client_point.y >= 0 && client_point.y < static_cast<LONG>(kHeaderHeight * scale) &&
        client_point.x >= 0 && client_point.x < client.right - static_cast<LONG>(70.0f * scale)) {
      POINT cursor{};
      RECT window_rect{};
      GetCursorPos(&cursor);
      GetWindowRect(hwnd, &window_rect);
  ...
  switch (message) {
    case WM_HOTKEY: {
      ...
      if (settings != nullptr && index >= 0 &&
          index < static_cast<int>(HotkeyAction::kCount) && settings->hotkey_action_callback_) {
