...
bool IsValidAnimationStyle(std::string_view value) {
  return value == "Classic Genie" || value == "Soft" || value == "Snappy" ||
         value == "Elastic" || value == "Linear";
}
...
public:
  ...
  bool Parse(AppSettings* settings) {
    ...
    while (true) {
      ...
      if (key == "enabled") {
        ...
        if (value_valid && (value == "No fade" || value == "Subtle" || value == "Strong")) {
          settings->fade_strength = std::move(value);
                 ...
                 key == "followWindowsAnimationPreference" ||
                 key == "startMinimized" ||
                 key == "runAtStartup") {
        bool value = false;
        value_valid = ParseBoolean(&value);
        ...
        if (value_valid && IsValidEasingName(value)) {
          if (key == "minimizeEasing") settings->minimize_easing = std::move(value);
          else settings->restore_easing = std::move(value);
        }
      ...
      } else if (const std::optional<HotkeyField> field = FindHotkeyField(key);
                 field.has_value()) {
        double value = 0.0;
        value_valid = ParseNumber(&value);
        if (value_valid && std::isfinite(value) && value == std::floor(value)) {
          if (field->is_modifier && value >= 0.0 &&
            ...
            settings->hotkeys[field->action_index].modifiers =
                static_cast<std::uint32_t>(value);
          } else if (!field->is_modifier && value >= 0.0 && value <= 254.0) {
            settings->hotkeys[field->action_index].virtual_key =
                static_cast<std::uint32_t>(value);
          }
        }
      } else {
...
private:
  ...
  void SkipWhitespace() {
    while (position_ < json_.size() &&
           (json_[position_] == ' ' || json_[position_] == '\t' || json_[position_] == '\r' ||
            json_[position_] == '\n')) {
      ++position_;
    }
  }
...
std::optional<std::string> NormalizeExecutableName(std::string_view name) {
  ...
  for (size_t i = 0; i < kExeSuffix.size(); ++i) {
    ...
    const char folded = ch >= 'A' && ch <= 'Z' ? static_cast<char>(ch + ('a' - 'A'))
                                               : static_cast<char>(ch);
...
bool ExecutableNamesEqual(std::string_view left, std::string_view right) {
  ...
  return CompareStringOrdinal(wide_left->data(), static_cast<int>(wide_left->size()),
                              wide_right->data(), static_cast<int>(wide_right->size()), TRUE) ==
         CSTR_EQUAL;
}
