...
extern "C" __declspec(dllexport) LRESULT CALLBACK CBTProc(int code, WPARAM w_param,
  ...
  if (code == HCBT_MINMAX) {
    ...
    if (IsMinimizeCommand(show_cmd)) {
      if (GetPropW(target_window, minimize::platform::windows::properties::kExcludedApplication) !=
          nullptr) {
        minimize::core::LogTrace(L"HookDLL", L"Minimize allowed natively for excluded application");
      } else if (GetPropW(target_window, minimize::platform::windows::properties::kAllowMinimize) ==
                 nullptr) {
        ...
        if (overlay_window != nullptr && message != 0) {
          if (PostMessageW(overlay_window, message, reinterpret_cast<WPARAM>(target_window),
                           l_param)) {
            minimize::core::LogTrace(L"HookDLL",
                                  L"PostMessage(MinimizeMinimizeAttempt) succeeded; blocking native "
                                  L"minimize");
            return 1;
          }
          minimize::core::LogDebug(L"HookDLL", L"PostMessage(MinimizeMinimizeAttempt) failed error=" +
                                                std::to_wstring(GetLastError()));
        }
      } else {
        minimize::core::LogTrace(L"HookDLL",
                              L"Minimize allowed by property MinimizeAllowMinimize for hwnd=0x" +
                                  std::to_wstring(reinterpret_cast<std::uintptr_t>(target_window)));
      }
    }
    ...
    if (IsRestoreCommand(show_cmd)) {
      if (GetPropW(target_window, minimize::platform::windows::properties::kExcludedApplication) !=
      ...
      } else if (GetPropW(target_window, minimize::platform::windows::properties::kAllowRestore) ==
                 nullptr) {
        HWND overlay_window = FindWindowW(kOverlayClassName, nullptr);
        ...
        if (overlay_window != nullptr && message != 0) {
          ...
          if (send_result == 0) {
            minimize::core::LogDebug(L"HookDLL",
                                  L"SendMessageTimeout(MinimizeRestoreAttempt) failed error=" +
                                      std::to_wstring(GetLastError()));
          }
          minimize::core::LogTrace(L"HookDLL", L"SendMessageTimeout(MinimizeRestoreAttempt) returned " +
        ...
        minimize::core::LogTrace(L"HookDLL",
                              L"Restore allowed by property MinimizeAllowRestore for hwnd=0x" +
                                  std::to_wstring(reinterpret_cast<std::uintptr_t>(target_window)));
      }
    }
  }
