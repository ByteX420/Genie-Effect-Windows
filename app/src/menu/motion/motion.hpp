...
namespace ui::motion {
  ...
  struct MotionSpec
  {
    ...
    static MotionSpec Timed( float durationSeconds, MotionEasing easingMode = MotionEasing::EaseOutQuad, float delaySeconds = 0.0f );
    static MotionSpec Spring( float responseStrength = 12.0f, MotionEasing springMode = MotionEasing::SpringSoft );
  };
  ...
  class MotionSystem {
   public:
    ...
    float value( std::string_view key, float target, const MotionSpec& spec, std::optional<float> initialValue = std::nullopt );
    float value( const MotionKey& key, float target, const MotionSpec& spec, std::optional<float> initialValue = std::nullopt );

    ImVec2 vec2( std::string_view key, const ImVec2& target, const MotionSpec& spec, std::optional<ImVec2> initialValue = std::nullopt );
    ImVec2 vec2( const MotionKey& key, const ImVec2& target, const MotionSpec& spec, std::optional<ImVec2> initialValue = std::nullopt );

    ImVec4 color( std::string_view key, const ImVec4& target, const MotionSpec& spec, std::optional<ImVec4> initialValue = std::nullopt );
    ImVec4 color( const MotionKey& key, const ImVec4& target, const MotionSpec& spec, std::optional<ImVec4> initialValue = std::nullopt );
   ...
   private:
    ...
    template <typename T>
    T animate( TrackMap<T>& tracks, std::string_view key, const T& target, const MotionSpec& spec, std::optional<T> initialValue );
