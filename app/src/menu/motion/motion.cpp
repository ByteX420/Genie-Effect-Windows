...
MotionKey::MotionKey(std::string_view rawKey) : storage(rawKey) {}
...
void MotionSystem::setReducedMotion(bool enabled) { reducedMotion_ = enabled; }

bool MotionSystem::reducedMotion() const { return reducedMotion_; }

float MotionSystem::value(std::string_view key, float target, const MotionSpec& spec,
                          std::optional<float> initialValue) {
  return animate(scalarTracks_, key, target, spec, initialValue);
}

float MotionSystem::value(const MotionKey& key, float target, const MotionSpec& spec,
                          std::optional<float> initialValue) {
  return value(key.value(), target, spec, initialValue);
}

ImVec2 MotionSystem::vec2(std::string_view key, const ImVec2& target, const MotionSpec& spec,
                          std::optional<ImVec2> initialValue) {
  return animate(vec2Tracks_, key, target, spec, initialValue);
}

ImVec2 MotionSystem::vec2(const MotionKey& key, const ImVec2& target, const MotionSpec& spec,
                          std::optional<ImVec2> initialValue) {
  return vec2(key.value(), target, spec, initialValue);
}

ImVec4 MotionSystem::color(std::string_view key, const ImVec4& target, const MotionSpec& spec,
                           std::optional<ImVec4> initialValue) {
  return animate(colorTracks_, key, target, spec, initialValue);
}

ImVec4 MotionSystem::color(const MotionKey& key, const ImVec4& target, const MotionSpec& spec,
                           std::optional<ImVec4> initialValue) {
...
void MotionSystem::set(const MotionKey& key, float value) { set(key.value(), value); }

void MotionSystem::set(const MotionKey& key, const ImVec2& value) { set(key.value(), value); }

void MotionSystem::set(const MotionKey& key, const ImVec4& value) { set(key.value(), value); }
...
void MotionSystem::forget(const MotionKey& key) { forget(key.value()); }

bool MotionSystem::isActive(std::string_view key) const {
  return isTrackActive(scalarTracks_, key) || isTrackActive(vec2Tracks_, key) ||
         isTrackActive(colorTracks_, key);
}

bool MotionSystem::isActive(const MotionKey& key) const { return isActive(key.value()); }
...
T MotionSystem::animate(TrackMap<T>& tracks, std::string_view key, const T& target,
                        const MotionSpec& spec, std::optional<T> initialValue) {
  ...
  if (IsSpring(track.spec.easing)) {
    ...
    if (track.elapsed < track.spec.delay) {
      ...
      const float progress =
          ImClamp((track.elapsed - track.spec.delay) / effectiveDuration, 0.0f, 1.0f);
