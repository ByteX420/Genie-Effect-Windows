...
namespace {
  inline std::string MotionIdString( ImGuiID id ) {
    return std::to_string( static_cast<ImU32>( id ) );
  }

  inline ui::motion::MotionKey MotionKeyFor( const char* scope, ImGuiID id, const char* channel ) {
    return ui::motion::MotionKey( scope, MotionIdString( id ), channel );
  }
}  // namespace
...
// - Read about ImTextureID here: https://github.com/ocornut/ui/imgui/wiki/Image-Loading-and-Displaying-Examples
...
namespace ImGui {
  static float s_CheckboxSpacing = 5.0f;  // Standardabstand zwischen Checkboxen

  void BeginCheckboxGroup( float spacing ) {
    s_CheckboxSpacing = spacing;
  }
  ...
  bool Checkbox( const char* label, bool* v ) {
    ...
    const ImVec2 pos       = window->DC.CursorPos;
    const ImRect total_bb( pos, pos + ImVec2( square_sz + ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f ), ImMax( square_sz, label_size.y ) ) );

    ItemSize( ImVec2( total_bb.GetWidth(), total_bb.GetHeight() + s_CheckboxSpacing ) );  // Zus tzlichen Platz f r den Abstand hinzuf gen
    ...
    const float  hoverT = motion.value( MotionKeyFor( "checkbox", id, "hover" ), hovered ? 1.0f : 0.0f, tokens.hoverFast, hovered ? 1.0f : 0.0f );
    const float  checkT = motion.value( MotionKeyFor( "checkbox", id, "check" ), *v ? 1.0f : 0.0f, tokens.pressFast, *v ? 1.0f : 0.0f );
    ...
    if ( hoverT > 0.0f ) {
      ImU32 hover_overlay = IM_COL32( 80, 80, 80, (int)( hoverT * 60.0f ) );
      window->DrawList->AddRectFilled( check_bb.Min, check_bb.Max, hover_overlay );
    }

    if ( checkT > 0.0f ) {
      ImVec4 checkedColor = colors::accent;
      checkedColor.w *= checkT;
      checkedColor.x = ImMin( checkedColor.x * ( 1.0f + checkT * 0.12f ), 1.0f );
      checkedColor.y = ImMin( checkedColor.y * ( 1.0f + checkT * 0.12f ), 1.0f );
      checkedColor.z = ImMin( checkedColor.z * ( 1.0f + checkT * 0.12f ), 1.0f );
      window->DrawList->AddRectFilled( check_bb.Min + ImVec2( 1, 1 ), check_bb.Max - ImVec2( 1, 1 ), ImGui::GetColorU32( checkedColor ) );
    }
...
bool ImGui::RadioButton( const char* label, bool active ) {
  ...
  ItemSize( ImVec2( total_bb.GetWidth(), total_bb.GetHeight() + s_CheckboxSpacing ) );
  if ( !ItemAdd( total_bb, id ) ) {
    IMGUI_TEST_ENGINE_ITEM_INFO( id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | ( active ? ImGuiItemStatusFlags_Checked : 0 ) );
    return false;
  }
   ...
   if ( pressed ) {
     // For radio buttons, pressing it means it becomes active
     // No need to toggle like checkbox, just set to active
     // MarkItemEdited(id); // MarkItemEdited is called in ButtonBehavior if pressed
   }
   ...
   const float radius  = ( square_sz - 1.0f ) * 0.5f;
    auto& motion  = WindowMotion::System();
   const auto& tokens  = WindowMotion::Tokens();
   const float  hoverT  = motion.value( MotionKeyFor( "radio", id, "hover" ), hovered ? 1.0f : 0.0f, tokens.hoverFast, hovered ? 1.0f : 0.0f );
   const float  fillT   = motion.value( MotionKeyFor( "radio", id, "fill" ), active ? 1.0f : 0.0f, tokens.pressFast, active ? 1.0f : 0.0f );
   ...
   if ( fillT > 0.0f ) {
     ImVec4 checkedColor = colors::accent;
     checkedColor.w *= fillT;
     checkedColor.x = ImMin( checkedColor.x * ( 1.0f + fillT * 0.12f ), 1.0f );
     checkedColor.y = ImMin( checkedColor.y * ( 1.0f + fillT * 0.12f ), 1.0f );
     checkedColor.z = ImMin( checkedColor.z * ( 1.0f + fillT * 0.12f ), 1.0f );
     window->DrawList->AddCircleFilled( center, radius * ( 0.55f + 0.05f * fillT ), ImGui::GetColorU32( checkedColor ), 24 );
   }
...
void ImGui::SeparatorEx( ImGuiSeparatorFlags flags, float thickness ) {
  ...
  if ( flags & ImGuiSeparatorFlags_Vertical ) {
    ...
    if ( ItemAdd( bb, 0 ) ) {
      std::string separatorId = "sep_" + std::to_string( (int)bb.Min.x ) + "_" + std::to_string( (int)bb.Min.y );
      const float fadeProgress = WindowMotion::System().value( ui::motion::MotionKey( "separator", separatorId, "alpha" ), 1.0f, WindowMotion::Tokens().fadeSlow, 0.0f );

      if ( fadeProgress > 0.0f ) {
        float totalWidth = bb.Max.x - bb.Min.x;
        float centerX    = bb.Min.x + totalWidth * 0.5f;

        float visibleHalfWidth = ( totalWidth * 0.5f ) * fadeProgress;

        float leftStart = centerX - visibleHalfWidth;
        float rightEnd  = centerX + visibleHalfWidth;

        leftStart = ImMax( leftStart, bb.Min.x );
        rightEnd  = ImMin( rightEnd, bb.Max.x );

        ImU32 baseSeparatorColor = GetColorU32( ImGuiCol_Separator );

        float baseR = ( ( baseSeparatorColor >> IM_COL32_R_SHIFT ) & 0xFF ) / 255.0f;
        float baseG = ( ( baseSeparatorColor >> IM_COL32_G_SHIFT ) & 0xFF ) / 255.0f;
        float baseB = ( ( baseSeparatorColor >> IM_COL32_B_SHIFT ) & 0xFF ) / 255.0f;
        float baseA = ( ( baseSeparatorColor >> IM_COL32_A_SHIFT ) & 0xFF ) / 255.0f;

        int blurPasses = 5;
        for ( int pass = 0; pass < blurPasses; pass++ ) {
          float passAlpha = baseA * ( 1.0f - (float)pass / (float)blurPasses ) * 0.8f;
          float passWidth = visibleHalfWidth * ( 1.0f + (float)pass * 0.1f );

          float passLeftStart = ImMax( centerX - passWidth, bb.Min.x );
          float passRightEnd  = ImMin( centerX + passWidth, bb.Max.x );

          if ( passRightEnd > passLeftStart ) {
            ImU32 passColor = IM_COL32(
                (int)( baseR * 255.0f ),
                (int)( baseG * 255.0f ),
                (int)( baseB * 255.0f ),
                (int)( passAlpha * 255.0f ) );

              window->DrawList->AddRectFilled(
                  ImVec2( passLeftStart, bb.Min.y ),
                  ImVec2( passRightEnd, bb.Max.y ),
                  passColor );
          }
        }
      }
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags ) {
  ...
  auto& motion     = WindowMotion::System();
  const auto&   tokens     = WindowMotion::Tokens();
  const std::string comboMotionId = MotionIdString( id );
  const std::string popupMotionId = MotionIdString( popup_id );
  if ( pressed && !popup_open ) {
    OpenPopupEx( popup_id, ImGuiPopupFlags_None );
    popup_open = true;
    motion.set( ui::motion::MotionKey( "combo", popupMotionId, "popup-alpha" ), 0.0f );
    motion.set( ui::motion::MotionKey( "combo", popupMotionId, "popup-slide" ), 0.0f );
  }

  // Render (flat, web-matched)
  const float value_x2 = ImMax( bb.Min.x, bb.Max.x - arrow_size );
  RenderNavHighlight( bb, id );

  ImVec4 normalFrame   = style.Colors[ImGuiCol_FrameBg];
  ImVec4 hoverFrame    = style.Colors[ImGuiCol_FrameBgHovered];
  ImVec4 animatedFrame = motion.color( ui::motion::MotionKey( "combo", comboMotionId, "frame" ), ( hovered || popup_open ) ? hoverFrame : normalFrame, tokens.hoverFast, normalFrame );

  window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( animatedFrame ), style.FrameRounding );
  RenderFrameBorder( bb.Min, bb.Max, style.FrameRounding );

  if ( !( flags & ImGuiComboFlags_NoArrowButton ) ) {
    ImVec4 arrow_col_v = motion.color( ui::motion::MotionKey( "combo", comboMotionId, "arrow" ), ( hovered || popup_open ) ? style.Colors[ImGuiCol_Text] : style.Colors[ImGuiCol_TextDisabled], tokens.hoverFast, style.Colors[ImGuiCol_TextDisabled] );
    ImU32  arrow_col   = GetColorU32( arrow_col_v );
    if ( value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x )
      RenderArrow( window->DrawList, ImVec2( value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y ), arrow_col, ImGuiDir_Down, 1.0f );
  }
...
bool ImGui::BeginComboPopup( ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags ) {
  ...
  const auto&   tokens   = WindowMotion::Tokens();
  const std::string popupMotionId = MotionIdString( popup_id );
  const float    popupAlpha = motion.value( ui::motion::MotionKey( "combo", popupMotionId, "popup-alpha" ), 1.0f, tokens.popupOpen, 0.0f );
  const float    popupSlide = motion.value( ui::motion::MotionKey( "combo", popupMotionId, "popup-slide" ), 1.0f, tokens.slideSoft, 0.0f );
  ...
  if ( ImGuiWindow* popup_window = FindWindowByName( name ) )
    if ( popup_window->WasActive ) {
      ...
      SetNextWindowPos( ImVec2( pos.x, pos.y + ( 1.0f - popupSlide ) * 6.0f ) );
    }

  // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
  ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
  PushStyleVar( ImGuiStyleVar_WindowPadding, ImVec2( g.Style.FramePadding.x, g.Style.WindowPadding.y ) );  // Horizontally align ourselves with the framed text
  PushStyleVar( ImGuiStyleVar_Alpha, popupAlpha );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
  ...
  const ImVec2 label_size    = CalcTextSize( label, label_end, true );
  const float  slider_height = label_size.y + 18.0f;
  const ImRect frame_bb( window->DC.CursorPos, window->DC.CursorPos + ImVec2( w, slider_height ) );
  ...
  // Default TempInput (Ctrl+Click on Slider)
  bool temp_input_is_active = temp_input_allowed && TempInputIsActive( id );
  if ( temp_input_is_active ) {
    const bool is_clamp_input = ( flags & ImGuiSliderFlags_AlwaysClamp ) != 0 || ( p_min != NULL && p_max != NULL );
    return TempInputScalar( frame_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL );
  }

  // --- Custom Value Box Geometry ---
  PushID( id );
  const char* value_label = "##v";
  ImGuiID     value_id    = window->GetID( value_label );
  PopID();

  char value_buf_pre[64];
  DataTypeFormatString( value_buf_pre, IM_ARRAYSIZE( value_buf_pre ), data_type, p_data, format );
  ImVec2 text_size_pre = CalcTextSize( value_buf_pre, NULL, false );

  // Fixed size value box (matches web: small mono input on the right)
  ImVec2 fixed_box_size = CalcTextSize( "1000", NULL, false );
  ImVec2 small_padding  = ImVec2( 3.0f, 2.0f );
  fixed_box_size.x = ImMax( fixed_box_size.x + small_padding.x * 2.0f, 32.0f );
  fixed_box_size.y = ImMax( fixed_box_size.y + small_padding.y * 2.0f, 18.0f );

  // Top-right aligned with label row
  ImVec2 value_box_pos = ImVec2( frame_bb.Max.x - fixed_box_size.x, frame_bb.Min.y );
  ImRect value_bb( value_box_pos, value_box_pos + fixed_box_size );

  // Check if value input is already active (using ActiveId instead of TempInputId)
  bool value_input_is_active = temp_input_allowed && ( g.ActiveId == value_id );

  // Check for hover on value box (only if not disabled)
  bool is_disabled   = ( g.CurrentItemFlags & ImGuiItemFlags_Disabled ) != 0;
  bool value_hovered = !is_disabled && value_bb.Contains( g.IO.MousePos );

  // Check for click on value box - activate input mode
  if ( temp_input_allowed && !value_input_is_active && value_hovered && g.IO.MouseClicked[0] ) {
    value_input_is_active = true;
  }

  // If value input is active (either from this frame or previous), handle it and return
  if ( value_input_is_active ) {
    const bool is_clamp_input = ( flags & ImGuiSliderFlags_AlwaysClamp ) != 0 || ( p_min != NULL && p_max != NULL );

    // Render slider track behind input (same geometry as normal mode)
    const ImRect line_bb(
        ImVec2( frame_bb.Min.x, frame_bb.Min.y + label_size.y + 6.0f ),
        ImVec2( frame_bb.Max.x, frame_bb.Min.y + label_size.y + 10.0f ) );
    const float line_h = line_bb.GetHeight();

    window->DrawList->AddRectFilled( line_bb.Min, line_bb.Max, IM_COL32( 34, 34, 34, 255 ), line_h * 0.5f );

    float grab_t = 0.0f;
    if ( data_type == ImGuiDataType_Float )
      grab_t = ScaleRatioFromValueT<float, float, float>( data_type, *(float*)p_data, *(const float*)p_min, *(const float*)p_max, ( flags & ImGuiSliderFlags_Logarithmic ) != 0, 0.0f, 0.0f );
    else if ( data_type == ImGuiDataType_S32 )
      grab_t = ScaleRatioFromValueT<ImS32, ImS32, float>( data_type, *(ImS32*)p_data, *(const ImS32*)p_min, *(const ImS32*)p_max, ( flags & ImGuiSliderFlags_Logarithmic ) != 0, 0.0f, 0.0f );
    else if ( data_type == ImGuiDataType_U32 )
      grab_t = ScaleRatioFromValueT<ImU32, ImS32, float>( data_type, *(ImU32*)p_data, *(const ImU32*)p_min, *(const ImU32*)p_max, ( flags & ImGuiSliderFlags_Logarithmic ) != 0, 0.0f, 0.0f );

    grab_t = ImSaturate( grab_t );
    const float track_start = line_bb.Min.x;
    const float track_end   = line_bb.Max.x;
    const float fill_end    = ImLerp( track_start, track_end, grab_t );

    ImU32 grab_col = GetColorU32( ImGuiCol_SliderGrab );
    window->DrawList->AddRectFilled( ImVec2( track_start, line_bb.Min.y ), ImVec2( fill_end, line_bb.Max.y ), grab_col, line_h * 0.5f );
    window->DrawList->AddCircleFilled( ImVec2( fill_end, ( line_bb.Min.y + line_bb.Max.y ) * 0.5f ), line_h * 0.65f + 1.0f, IM_COL32( 255, 255, 255, 220 ) );

    if ( label_size.x > 0.0f ) {
      PushStyleColor( ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled] );
      RenderText( ImVec2( frame_bb.Min.x, value_bb.Min.y ), label, label_end );
      PopStyleColor();
    }

    // Draw value box background (stays visible during input)
    ImU32 value_bg     = GetColorU32( ImGuiCol_WindowBg );
    ImU32 value_border = GetColorU32( ImGuiCol_CheckMark );
    window->DrawList->AddRectFilled( value_bb.Min, value_bb.Max, value_bg, style.FrameRounding );
    window->DrawList->AddRect( value_bb.Min, value_bb.Max, value_border, style.FrameRounding );

    // Format current value for editing
    char buf[64];
    DataTypeFormatString( buf, IM_ARRAYSIZE( buf ), data_type, p_data, format );

    // Check if we have active input state - use that for live centering
    ImVec2               current_text_size;
    ImGuiInputTextState* input_state = GetInputTextState( value_id );
    if ( input_state && input_state->ID == value_id ) {
      // Use live editing text for centering
      current_text_size = CalcTextSize( input_state->TextA.Data, input_state->TextA.Data + input_state->CurLenA, false );
    } else {
      // Fallback to formatted value
      current_text_size = CalcTextSize( buf, NULL, false );
    }

    // Save complete layout state before input (InputTextEx modifies these)
    ImGuiWindowTempData backup_dc = window->DC;

    // Calculate centering: position the input box so its text appears centered
    float content_width = fixed_box_size.x - small_padding.x * 2.0f;
    float center_offset = ( content_width - current_text_size.x ) * 0.5f;
    if ( center_offset < 0.0f )
      center_offset = 0.0f;

    window->DC.CursorPos = ImVec2(
        value_bb.Min.x + small_padding.x + center_offset,
        value_bb.Min.y + small_padding.y );

    PushID( id );
    PushStyleVar( ImGuiStyleVar_FramePadding, ImVec2( 0.0f, 0.0f ) );
    PushStyleColor( ImGuiCol_FrameBg, IM_COL32( 0, 0, 0, 0 ) );  // Transparent since we drew bg above
    PushStyleColor( ImGuiCol_TextSelectedBg, IM_COL32( 100, 100, 100, 100 ) );

    // Use a size that fits current content to prevent clipping
    ImGuiInputTextFlags input_flags   = ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_CharsDecimal;
    bool                enter_pressed = InputTextEx( value_label, NULL, buf, 16, ImVec2( fixed_box_size.x - small_padding.x * 2.0f, fixed_box_size.y - small_padding.y * 2.0f ), input_flags );

    PopStyleColor( 2 );
    PopStyleVar();
    PopID();

    // Restore complete layout state
    window->DC = backup_dc;

    // Apply value if Enter was pressed or focus lost
    bool value_changed = false;
    if ( enter_pressed || ( g.ActiveId != value_id && g.ActiveIdPreviousFrame == value_id ) ) {
      DataTypeApplyFromText( buf, data_type, p_data, format, NULL );
      if ( is_clamp_input && p_min && p_max )
  ...
  // Precise interaction area for slider dragging (centered on the track)
  const float  interact_padding_y = 3.0f;  // Minimal padding for a tight feel
  const ImRect slider_interact_bb(
      ImVec2( frame_bb.Min.x, line_bb.Min.y - interact_padding_y ),
      ImVec2( frame_bb.Max.x, line_bb.Max.y + interact_padding_y ) );

  // --- Normal Slider Mode (Interaction Check) ---
  const bool hovered = ItemHoverable( slider_interact_bb, id, g.LastItemData.InFlags );
  const bool clicked = hovered && IsMouseClicked( 0, ImGuiInputFlags_None, id );
  if ( ( clicked || g.NavActivateId == id ) && !value_hovered ) {
    SetActiveID( id, window );
    SetFocusID( id, window );
    FocusWindow( window );
    g.ActiveIdUsingNavDirMask |= ( 1 << ImGuiDir_Left ) | ( 1 << ImGuiDir_Right );
  ...
  // Run SliderBehavior with the tight interaction area
  ImRect grab_bb;
  bool   value_changed = SliderBehavior( slider_interact_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb );
  if ( value_changed )
    MarkItemEdited( id );
  ...
  if ( grab_bb.Max.x > grab_bb.Min.x ) {
    ...
    float* p_last_t = window->StateStorage.GetFloatRef( id, -1000.0f );
    if ( *p_last_t == -1000.0f )
      *p_last_t = grab_t;
    *p_last_t = ImLerp( *p_last_t, grab_t, g.IO.DeltaTime * 24.0f );
    grab_t    = ImSaturate( *p_last_t );
  ...
  // Draw value box (not in input mode) - text centered
  ImU32 value_bg = GetColorU32( ImGuiCol_WindowBg );
  ImU32 value_border = GetColorU32( value_hovered ? ImGuiCol_TextDisabled : ImGuiCol_Border );
  window->DrawList->AddRectFilled( value_bb.Min, value_bb.Max, value_bg, style.FrameRounding );
  window->DrawList->AddRect( value_bb.Min, value_bb.Max, value_border, style.FrameRounding );
  RenderTextClipped( value_bb.Min, value_bb.Max, value_buf_pre, NULL, &text_size_pre, ImVec2( 0.5f, 0.5f ) );
...
bool ImGui::SliderFloat( const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags ) {
  ...
  bool result = SliderScalar( label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags );
  if ( *v < v_min )
    *v = v_min;
  if ( *v > v_max )
    *v = v_max;
...
bool ImGui::InputTextEx( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data ) {
  ...
  if ( render_cursor || render_selection ) {
    ...
    if ( render_cursor ) {
      ...
      // Smooth Apple-like blink (sine wave fade out/in, solid when typing)
      float blink_alpha = 1.0f;
      if ( g.IO.ConfigInputTextCursorBlink && state->CursorAnim > 0.0f ) {
        float t = ImFmod( state->CursorAnim, 1.20f );
        if ( t < 0.50f ) {
          blink_alpha = 1.0f; // Hold solid
        } else if ( t < 0.85f ) {
          blink_alpha = 1.0f - ( t - 0.50f ) / 0.35f; // Smooth fade out
        } else {
          blink_alpha = ( t - 0.85f ) / 0.35f; // Smooth fade in
        }
      }
      ...
      auto& motion = WindowMotion::System();
      const auto& tokens = WindowMotion::Tokens();
      float target_cursor_x = cursor_offset.x;
      
      // We use a specific motion key for this text input's cursor
      float visual_cursor_x = motion.value( 
          MotionKeyFor( "input_cursor", id, "x" ), 
          target_cursor_x, 
          tokens.springSnappy, 
          target_cursor_x 
      );

      // Snap if distance is huge (e.g., initialized or cleared)
      if ( std::abs( visual_cursor_x - target_cursor_x ) > g.FontSize * 15.0f ) {
        motion.set( MotionKeyFor( "input_cursor", id, "x" ), target_cursor_x );
        visual_cursor_x = target_cursor_x;
      }

      ImVec2 cursor_screen_pos = ImTrunc( draw_pos + ImVec2( visual_cursor_x, cursor_offset.y ) - draw_scroll );
      const float cursor_height = g.FontSize - 2.0f; // Thin and slightly shorter than full text bounds
      // Push the cursor down by +1.5px from the top of the font height to properly center it visually
      const float cursor_top = ImTrunc( cursor_screen_pos.y - g.FontSize + 1.5f ); 
      const float cursor_bottom = cursor_top + cursor_height;
      ImRect cursor_screen_rect( cursor_screen_pos.x, cursor_top, cursor_screen_pos.x + 1.5f, cursor_bottom ); // 1.5px width for sharp but visible
      
      if ( blink_alpha > 0.01f && cursor_screen_rect.Overlaps( clip_rect ) ) {
        ImU32 cursor_col = ImGui::GetColorU32( ImVec4( 1.0f, 1.0f, 1.0f, blink_alpha ) ); // White cursor
        // Clean, sharp, flat cursor without rounded corners
        draw_window->DrawList->AddRectFilled( cursor_screen_rect.Min, cursor_screen_rect.Max, cursor_col, 0.0f );
      }
...
bool ImGui::ColorPicker4( const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col ) {
  ...
  const bool horizontal_bars    = ( flags & ImGuiColorEditFlags_PickerHueBar ) != 0;
  float  bars_width             = square_sz;
  float  bars_height            = ImMax( 4.0f, square_sz * 0.5f );
  float  sv_picker_size         = horizontal_bars ? width : ImMax( bars_width * 1, width - ( alpha_bar ? 2 : 1 ) * ( bars_width + style.ItemInnerSpacing.x ) );
  float  bar0_pos_x             = horizontal_bars ? picker_pos.x : picker_pos.x + sv_picker_size + style.ItemInnerSpacing.x;
  float  bar1_pos_x             = horizontal_bars ? picker_pos.x : bar0_pos_x + bars_width + style.ItemInnerSpacing.x;
  float  bar0_pos_y             = picker_pos.y + sv_picker_size + style.ItemInnerSpacing.y;
  float  bar1_pos_y             = bar0_pos_y + bars_height + style.ItemInnerSpacing.y;
  ...
  if ( flags & ImGuiColorEditFlags_PickerHueWheel ) {
    ...
    // Thin horizontal hue bar below the SV rectangle.
    SetCursorScreenPos( ImVec2( bar0_pos_x, bar0_pos_y ) );
    InvisibleButton( "hue", ImVec2( sv_picker_size, bars_height ) );
    if ( IsItemActive() && !is_readonly ) {
  ...
  // Alpha bar logic
  if ( alpha_bar ) {
    SetCursorScreenPos( horizontal_bars ? ImVec2( bar1_pos_x, bar1_pos_y ) : ImVec2( bar1_pos_x, picker_pos.y ) );
    InvisibleButton( "alpha", horizontal_bars ? ImVec2( sv_picker_size, bars_height ) : ImVec2( bars_width, sv_picker_size ) );
    if ( IsItemActive() ) {
      col[3]        = horizontal_bars
                          ? ImSaturate( ( io.MousePos.x - picker_pos.x ) / ( sv_picker_size - 1 ) )
                          : 1.0f - ImSaturate( ( io.MousePos.y - picker_pos.y ) / ( sv_picker_size - 1 ) );
      value_changed = true;
    }
  }
  ...
  if ( flags & ImGuiColorEditFlags_PickerHueWheel ) {
    ...
    // Render the thin horizontal hue bar below the SV square.
    const ImVec2 hueBarMin( bar0_pos_x, bar0_pos_y );
    const ImVec2 hueBarMax( bar0_pos_x + sv_picker_size, bar0_pos_y + bars_height );
    for ( int i = 0; i < 6; ++i ) {
      const float x0 = bar0_pos_x + i * ( sv_picker_size / 6.0f );
      const float x1 = bar0_pos_x + ( i + 1 ) * ( sv_picker_size / 6.0f );
      draw_list->AddRectFilledMultiColor( ImVec2( x0, bar0_pos_y ), ImVec2( x1, bar0_pos_y + bars_height ), col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i] );
    }
    RenderFrameBorder( hueBarMin, hueBarMax, 0.0f );

    const float hueHandleX = IM_ROUND( picker_pos.x + H * sv_picker_size );
    const float handleHalfW = ImMax( 2.0f, bars_height * 0.22f );
    draw_list->AddRectFilled( ImVec2( hueHandleX - handleHalfW - 1.0f, bar0_pos_y - 2.0f ), ImVec2( hueHandleX + handleHalfW + 1.0f, bar0_pos_y + bars_height + 2.0f ), col_midgrey, 2.0f );
    draw_list->AddRectFilled( ImVec2( hueHandleX - handleHalfW, bar0_pos_y - 1.0f ), ImVec2( hueHandleX + handleHalfW, bar0_pos_y + bars_height + 1.0f ), col_white, 2.0f );
  }
  ...
  if ( alpha_bar ) {
    ...
    if ( horizontal_bars ) {
      ImRect bar1_bb( bar1_pos_x, bar1_pos_y, bar1_pos_x + sv_picker_size, bar1_pos_y + bars_height );
      RenderColorRectWithAlphaCheckerboard( draw_list, bar1_bb.Min, bar1_bb.Max, 0, ImMax( 2.0f, bars_height * 0.5f ), ImVec2( 0.0f, 0.0f ) );
      const ImU32 transparent = user_col32_striped_of_alpha & ~IM_COL32_A_MASK;
      draw_list->AddRectFilledMultiColor( bar1_bb.Min, bar1_bb.Max, transparent, user_col32_striped_of_alpha, user_col32_striped_of_alpha, transparent );
      RenderFrameBorder( bar1_bb.Min, bar1_bb.Max, 0.0f );

      const float alphaHandleX = IM_ROUND( picker_pos.x + alpha * sv_picker_size );
      const float handleHalfW  = ImMax( 2.0f, bars_height * 0.22f );
      draw_list->AddRectFilled( ImVec2( alphaHandleX - handleHalfW - 1.0f, bar1_pos_y - 2.0f ), ImVec2( alphaHandleX + handleHalfW + 1.0f, bar1_pos_y + bars_height + 2.0f ), col_midgrey, 2.0f );
      draw_list->AddRectFilled( ImVec2( alphaHandleX - handleHalfW, bar1_pos_y - 1.0f ), ImVec2( alphaHandleX + handleHalfW, bar1_pos_y + bars_height + 1.0f ), col_white, 2.0f );
    } else {
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size_arg ) {
  ImGuiWindow* window = GetCurrentWindow();
  if ( window->SkipItems )
    return false;
  ...
  float         checkbox_size = GetFrameHeight() * 0.7f;  // Dies entspricht der Gr  e der Checkbox

  // Anpassung der Gr  e: Vertikale H he wie Checkbox, horizontale Breite doppelt so gro
  if ( size_arg.x == 0.0f )
    size_arg.x = checkbox_size * 2;  // Doppelt so breit wie die Checkbox
  if ( size_arg.y == 0.0f )
    size_arg.y = checkbox_size;  // Gleiche H he wie die Checkbox

  const ImVec2 size( size_arg.x, size_arg.y );
  const ImRect bb( window->DC.CursorPos, window->DC.CursorPos + size );
  ...
  // Render the color button in gamesense style
  const float rounding   = 0.0f;  // No rounding for a flat look
  const ImU32 col_bg     = GetColorU32( hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg );
  const ImU32 col_border = GetColorU32( ImGuiCol_Border );

  // Draw background
  // window->DrawList->AddRectFilled(bb.Min, bb.Max, col_bg, rounding);

  window->DrawList->AddRectFilledMultiColor(
      bb.Min,
      bb.Max,
      IM_COL32( 50, 50, 50, 200 ),  // Sehr dunkles Grau links (etwas deckend)
      IM_COL32( 40, 40, 40, 150 ),  // Noch dunkleres Grau rechts (etwas transparenter)
      IM_COL32( 20, 20, 20, 100 ),  // Noch dunklerer Mittelstreifen (noch transparenter)
      IM_COL32( 40, 40, 40, 150 )   // Noch dunkleres Grau rechts (etwas transparenter)
  );

  // Draw color
  if ( flags & ( ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf ) ) {
    const float grid_step = ImMin( size_arg.x, size_arg.y ) / 2.99f;
    const float rounding  = 0.0f;
    ImRect      bb_inner( bb.Min + ImVec2( 1, 1 ), bb.Max - ImVec2( 1, 1 ) );
    if ( flags & ImGuiColorEditFlags_AlphaPreviewHalf ) {
      float w = bb_inner.GetWidth();
      ImRect bb_half( bb_inner.Min, bb_inner.Max );
      bb_half.Max.x = bb_half.Min.x + w * 0.5f;
      // Draw opaque on the right side
      window->DrawList->AddRectFilled( bb_inner.Min, bb_inner.Max, GetColorU32( col_rgb_without_alpha ), rounding );
      // Draw checkerboard + alpha on the left side
      RenderColorRectWithAlphaCheckerboard( window->DrawList, bb_half.Min, bb_half.Max, GetColorU32( col_rgb ), grid_step, ImVec2( 0.0f, 0.0f ), rounding );
    } else {
      RenderColorRectWithAlphaCheckerboard( window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32( col_rgb ), grid_step, ImVec2( 0.0f, 0.0f ), rounding );
    }
  } else {
    window->DrawList->AddRectFilled( bb.Min + ImVec2( 1, 1 ), bb.Max - ImVec2( 1, 1 ), GetColorU32( col_rgb_without_alpha ), rounding );
  }

  window->DrawList->AddRectFilledMultiColor(
      bb.Min,
      bb.Max,
      IM_COL32( 50, 50, 50, 100 ),  // Sehr dunkles Grau links (etwas deckend)
      IM_COL32( 40, 40, 40, 75 ),   // Noch dunkleres Grau rechts (etwas transparenter)
      IM_COL32( 20, 20, 20, 50 ),   // Noch dunklerer Mittelstreifen (noch transparenter)
      IM_COL32( 40, 40, 40, 75 )    // Noch dunkleres Grau rechts (etwas transparenter)
  );
...
bool ImGui::TreeNodeBehavior( ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end ) {
  ...
  // In this branch, TreeNodeBehavior() cannot toggle the selection so this will never trigger.
  // if (*p_selected != was_selected) //-V547
...
IMGUI_API bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg ) {
  // We use the same sets of flags as ButtonBehavior().
  return Selectable( label, &selected, flags, size_arg );
}

IMGUI_API bool ImGui::Selectable( const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg ) {
  ImGuiWindow* window = GetCurrentWindow();
  if ( window->SkipItems )
    return false;
  ...
  // Custom: Handle hover animation for selectable items
  bool             hovered, held;
  ImGuiButtonFlags button_flags_behavior = 0;
  if ( flags & ImGuiSelectableFlags_NoHoldingActiveID ) {
    button_flags_behavior |= ImGuiButtonFlags_NoHoldingActiveId;
  }
  if ( flags & ImGuiSelectableFlags_NoSetKeyOwner ) {
    button_flags_behavior |= ImGuiButtonFlags_NoSetKeyOwner;
  }
  if ( flags & ImGuiSelectableFlags_SelectOnClick ) {
    button_flags_behavior |= ImGuiButtonFlags_PressedOnClick;
  }
  if ( flags & ImGuiSelectableFlags_SelectOnRelease ) {
    button_flags_behavior |= ImGuiButtonFlags_PressedOnRelease;
  }
  if ( flags & ImGuiSelectableFlags_AllowDoubleClick ) {
    button_flags_behavior |= ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnDoubleClick;
  }
  if ( ( flags & ImGuiSelectableFlags_AllowOverlap ) || ( g.LastItemData.InFlags & ImGuiItemFlags_AllowOverlap ) ) {
    button_flags_behavior |= ImGuiButtonFlags_AllowOverlap;
  }

  bool pressed = ButtonBehavior( bb, id, &hovered, &held, button_flags_behavior );

  auto& motion = WindowMotion::System();
  const auto& tokens = WindowMotion::Tokens();
  const std::string selectableMotionId = MotionIdString( id );
  const float hoverT = motion.value( ui::motion::MotionKey( "selectable", selectableMotionId, "hover" ), hovered ? 1.0f : 0.0f, tokens.hoverFast, hovered ? 1.0f : 0.0f );
  const float pressT = motion.value( ui::motion::MotionKey( "selectable", selectableMotionId, "press" ), held ? 1.0f : 0.0f, tokens.pressFast, 0.0f );

  ImVec4 normal_frame_color = ImGui::GetStyleColorVec4( ImGuiCol_FrameBg );
  ImVec4 hover_frame_color  = ImGui::GetStyleColorVec4( ImGuiCol_FrameBgHovered );
  ImVec4 normal_text_color  = ImGui::GetStyleColorVec4( ImGuiCol_Text );
  ImVec4 hover_text_color   = ImGui::GetStyleColorVec4( ImGuiCol_TextSelectedBg );

  ImVec4 animated_frame_color;
  animated_frame_color.x = normal_frame_color.x + ( hover_frame_color.x - normal_frame_color.x ) * hoverT;
  animated_frame_color.y = normal_frame_color.y + ( hover_frame_color.y - normal_frame_color.y ) * hoverT;
  animated_frame_color.z = normal_frame_color.z + ( hover_frame_color.z - normal_frame_color.z ) * hoverT;
  animated_frame_color.w = normal_frame_color.w + ( hover_frame_color.w - normal_frame_color.w ) * hoverT;

  ImVec4 click_frame_color = ImVec4( 0.5f, 0.5f, 0.5f, 1.0f );
  animated_frame_color.x = animated_frame_color.x + ( click_frame_color.x - animated_frame_color.x ) * pressT;
  animated_frame_color.y = animated_frame_color.y + ( click_frame_color.y - animated_frame_color.y ) * pressT;
  animated_frame_color.z = animated_frame_color.z + ( click_frame_color.z - animated_frame_color.z ) * pressT;
  animated_frame_color.w = animated_frame_color.w + ( click_frame_color.w - animated_frame_color.w ) * pressT;

  ImVec4 animated_text_color;
  animated_text_color.x = normal_text_color.x + ( hover_text_color.x - normal_text_color.x ) * hoverT;
  animated_text_color.y = normal_text_color.y + ( hover_text_color.y - normal_text_color.y ) * hoverT;
  animated_text_color.z = normal_text_color.z + ( hover_text_color.z - normal_text_color.z ) * hoverT;
  animated_text_color.w = normal_text_color.w + ( hover_text_color.w - normal_text_color.w ) * hoverT;

  ImVec4 click_text_color = ImVec4( 0.5f, 0.5f, 0.5f, 1.0f );
  animated_text_color.x = animated_text_color.x + ( click_text_color.x - animated_text_color.x ) * pressT;
  animated_text_color.y = animated_text_color.y + ( click_text_color.y - animated_text_color.y ) * pressT;
  animated_text_color.z = animated_text_color.z + ( click_text_color.z - animated_text_color.z ) * pressT;
  animated_text_color.w = animated_text_color.w + ( click_text_color.w - animated_text_color.w ) * pressT;

  // Render frame
  RenderFrame( bb.Min, bb.Max, GetColorU32( animated_frame_color ), true, style.FrameRounding );

  // Render text with animated color
  PushStyleColor( ImGuiCol_Text, GetColorU32( animated_text_color ) );
  RenderTextClipped( text_min, text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb );
  PopStyleColor();

  bool was_selected = ( p_selected ? *p_selected : false );
  ...
  // Automatically close popups
  if ( pressed && ( window->Flags & ImGuiWindowFlags_Popup ) && !( flags & ImGuiSelectableFlags_DontClosePopups ) && !( g.LastItemData.InFlags & ImGuiButtonFlags_DontClosePopups ) )
    CloseCurrentPopup();
  ...
  // Render
  if ( hovered || ( p_selected && *p_selected ) ) {
...
// - others https://github.com/ocornut/ui/imgui/wiki/Useful-Extensions
//-------------------------------------------------------------------------
